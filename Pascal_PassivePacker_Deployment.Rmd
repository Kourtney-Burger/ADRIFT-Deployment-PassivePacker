---
title: "PASCAL Specific PassivePacker Deployment"
...
2.  Download unpackaged drifts from [deployment details spreadsheet](https://docs.google.com/spreadsheets/d/10bxlwfVOe1LFfj69B_YddxcA0V14m7codYwgD2YncFk/edit#gid=395545420) and save as `PASCAL_DeploymentDetails.csv`
3.  Import and manipulate data by following the code below

## Import Deployment Details

```{r}
deployDetails <- read.csv(here("DeploymentDetailsFiles","PASCAL_DeploymentDetails.csv"))
```

## Remove Unusable Drifts

```{r}
# Subset target deployments 
deployDetails <- subset(deployDetails, Project == "PASCAL")
...
deployDetails[deployDetails == "SM3M"] <- "Song Meter SM3M"
deployDetails[deployDetails == "SM2Bat"] <- "Song Meter SM2+"
```
...
dd <- dd %>%
  add_column(ID = NA, SITE_CRUISE = NA, PACKAGE_ID = NA, SOURCE_PATH = NA,
             ...
             AUDIO_END_TIME = NA, TITLE = "Passive Acoustics Survey of Cetacean Abundance Levels (PASCAL-2016)", 
...
# Set publication date to date packaged
dd$PUBLICATION_DATE <- '2024-02-06'
...
# Dataset Description
dd$PURPOSE <- 'The Passive Acoustics Survey of Cetacean Abundance Levels (PASCAL) was conducted by the National Marine Fisheries Service - Southwest Fisheries Science Center (SWFSC) to assess cetacean population abundance levels throughout the California Current study area off the U.S. West Coast. Targeting beaked whales (family Ziphiidae), sperm whales (Physeter macrocephalus), and dwarf and pygmy sperm whales (genus Kogia), this was a large scale acoustic survey using drifting acoustic buoys to obtain improved data for estimating population size for these species. This survey also aimed to imrpove our ability to identify species specific acoustic calls. Lastly, a pilot experiment was conducted as a part of this survey to deploy acoustic instruments near seamounts and explore the hypothesis that beaked whale occurrence is associated with these topographic seafloor features.'

dd$ABSTRACT <- 'These data were primarily collected using Drifting Acoustic Spar Buoy Recorders (DASBRs), which were deployed at 20 pre-determined locations distributed uniformly throughout the California Current Study area offshore of the continental shelf. Each DASBR was composed of at least two hydrophones at 100 meter depth and an acoustic recorder (several drifts had more hydrophones and two recorders). While the primary species of interests were beaked whales, sperm whales, and Kogia, these data have been analyzed for acoustic presence of a wide variety of cetacean species as well as soundscape analysis.'
